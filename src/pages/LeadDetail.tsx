...
import { fetchLeadById, updateLead, updateLeadStage, addConversationLog, updateConversationLog, deleteConversationLog, convertLead, fetchBookingsByLead } from '../api';
import { useState } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { ArrowLeft, Phone, MessageCircle, Users, Clock, Edit2, CheckCircle, X, ExternalLink } from 'lucide-react';

const STAGES = ['New', 'Contacted', 'Sale Closed', 'Bookinged', 'Active Customer', 'Lost'] as const;
...
const LeadDetail = () => {
  ...
  const editLogMutation = useMutation({
    mutationFn: ({ logId, note }: { logId: string; note: string }) => updateConversationLog(id!, logId, note),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['lead', id] });
  ...
  const channelIcon = lead.channel === 'Messenger' || lead.channel === 'Viber'
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/leads')} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-all">
            <ArrowLeft size={20} />
          </button>
          <div>
            <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Lead Detail</h1>
        ...
        <div className="flex items-center gap-2">
          {lead.stage === 'Bookinged' && leadBookings.length > 0 && (
            ...
            >
              <ExternalLink size={16} /> View Booking ({leadBookings[0].bookingNumber})
            </button>
          )}
          {lead.stage !== 'Active Customer' && lead.stage !== 'Lost' && lead.stage !== 'Bookinged' && (
            <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
              <h2 className="text-sm font-extrabold text-gray-900 uppercase tracking-wide">Lead Information</h2>
              <button
                onClick={() => {
                  if (isEditing) {
                    setIsEditing(false);
            ...
            <div className="p-6 space-y-4">
              {isEditing ? (
                <>
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-0.5">Name</label>
                    <input
                      type="text"
                ...
                <>
                  ...
                  <div>
                    ...
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-bold ${STAGE_COLORS[lead.stage] || 'bg-gray-100 text-gray-600'}`}>
                      {lead.stage}
                    </span>
                  </div>
                  {lead.assignedStaffName && (
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-sm text-gray-900">{lead.assignedStaffName}</span>
                      </div>
                    </div>
                  )}
                  {lead.requirements && (
                  ...
                  <div className="pt-2 border-t border-gray-100">
                    ...
                    <p className="text-xs text-gray-400">Updated: {new Date(lead.updatedAt).toLocaleDateString()}</p>
                  </div>
                </>
              )}
            </div>
          ...
          {!isEditing && lead.stage !== 'Bookinged' && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <h2 className="text-sm font-extrabold text-gray-900 uppercase tracking-wide">Change Stage</h2>
              </div>
              <div className="p-6">
                <select
                  value={lead.stage}
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
              <h2 className="text-sm font-extrabold text-gray-900 uppercase tracking-wide">Conversation Logs</h2>
            </div>
            <div className="p-6">
              {/* Timeline */}
              <div className="space-y-4 mb-6">
                {lead.conversationLogs && lead.conversationLogs.length > 0 ? (
                  [...lead.conversationLogs].reverse().map((log: any, idx: number) => {
                    const originalIndex = lead.conversationLogs.length - 1 - idx;
                    return (
                    <div key={originalIndex} className={`group relative pl-6 pb-4 border-l-2 last:border-l-0 ${log.isDeleted ? 'border-gray-200' : 'border-primary/20'}`}>
                      <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 flex items-center justify-center ${log.isDeleted ? 'bg-gray-200 border-gray-300' : 'bg-primary/20 border-primary'}`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${log.isDeleted ? 'bg-gray-400' : 'bg-primary'}`} />
                      </div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-bold text-gray-900">{log.staffName}</span>
                        ...
                        {!log.isDeleted && (
                          <div className="flex items-center gap-1 ml-auto opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={() => { setEditingLogId(String(originalIndex)); setEditingNote(log.note); }}
                      ...
                      {editingLogId === String(originalIndex) ? (
                        <div className="mt-1">
                          ...
                          <div className="flex gap-2 mt-1">
                            <button
                              onClick={() => editLogMutation.mutate({ logId: String(originalIndex), note: editingNote })}
                              disabled={!editingNote.trim() || editLogMutation.isPending}
                              className="px-3 py-1 bg-primary text-white text-xs font-bold rounded-lg hover:bg-primary-dark disabled:opacity-50"
                            >
                              Save
                            </button>
                            <button
                              onClick={() => { setEditingLogId(null); setEditingNote(''); }}
                  ...
                  <div className="text-center py-8 text-gray-400 text-sm">No conversation logs yet</div>
                )}
              </div>
              ...
              {deleteLogId && (
                <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center">
                  <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
                    <h3 className="text-lg font-extrabold text-gray-900 mb-2">Delete Message</h3>
                    <p className="text-sm text-gray-500 mb-6">This message will be replaced with "This message was deleted".</p>
                    <div className="flex gap-3">
                      <button
                      ...
                      <button
                        onClick={() => { deleteLogMutation.mutate(deleteLogId); setDeleteLogId(null); }}
