...
import { FileText, BarChart3, Users, HeartHandshake, History, PhoneCall, CalendarDays, CalendarSearch, Banknote, PieChart, ClipboardList } from 'lucide-react';
...
const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-14 left-0 h-[calc(100vh-3.5rem)] w-64 bg-[#148f73] shadow-lg z-40 transform transition-transform duration-300 ease-in-out overflow-y-auto ${
      ...
      >
        ...
        <nav className="py-4 space-y-1">
          {navItems.map((item) => {
            if (!item.roles.includes(isAdmin ? 'admin' : user?.role || '')) return null;
            ...
            return (
              ...
              >
                ...
                {'isNew' in item && item.isNew && (
                  <span className="ml-auto text-[9px] font-bold bg-white text-[#1CB89B] px-1.5 py-0.5 rounded-full">NEW</span>
                )}
              </Link>
            );
          })}
